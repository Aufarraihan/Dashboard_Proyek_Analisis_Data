...
def create_sum_order_items_df(df):
    sum_order_items_df = df.groupby("product_category_name_english").price_y.sum().sort_values(ascending=False).reset_index()
...
def create_rfm_df(df):
    rfm_df = df.groupby(by="customer_id_x", as_index=False).agg({
        "order_approved_at_y": "max", #mengambil tanggal order terakhir
        "order_id": "nunique",
        "price_y": "sum"
    })
...
all_df["order_approved_at_y"] = pd.to_datetime(all_df["order_approved_at_y"])
...
with st.sidebar:
    ...
    st.image("C:/Users/Aufar Raihan Rahmat/Downloads/Kuliah/Dioding/Proyek_Analisis_Data/assets/logo.png")
...
main_df = all_df[(all_df["order_approved_at_x"] >= str(start_date)) & (all_df["order_approved_at_x"] <= str(end_date))]
...
st.header('Brazil Marketplace Dashboard :sparkles:')
...
sns.barplot(x="price_y", y="product_category_name_english", data=sum_order_items_df.head(5), palette=colors, ax=ax[0])
...
sns.barplot(x="price_y", y="product_category_name_english", data=sum_order_items_df.sort_values(by="price_y", ascending=True).head(5), palette=colors, ax=ax[1])
...
ax[0].set_xticklabels(ax[0].get_xticklabels(), rotation=315, ha="left")
...
ax[2].set_xticklabels(ax[2].get_xticklabels(), rotation=315, ha="left")
 
st.pyplot(fig)
 
st.caption('Copyright (c) BM 2025')